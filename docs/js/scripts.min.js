...
/**
 * @author github.com/adjoo
 * Управляет разметкой, обеспечивая постраничный скролл
 * Требует
 * Требует следующую разметку основной области:
 * <wrapper> //неподвижный блок, position: fixed
 *     <section/> //страница, подлежащая постраничному скроллу
 *     <section/>
 *     <section/>
 * </wrapper>
 * @requires jQuery
 * @requires TouchSwipe - jQuery Plugin https://github.com/mattbryson/TouchSwipe-Jquery-Plugin
 * @param {options.markersContainerSelector} селектор неподвижного контейнера, default=='.fullPageScroll__container'.
 * @param {options.pageSelector} селектор подвижного контейнера, default=='.fullPageScroll__section'.
 * @param {options.markerSelector} селектор навигационного меню, default=='.fullPageScroll__scrollButton'.
 * @param {options.initialActivePage} начальная страница, default==0.
 * @returns добавляет класс 'active' к текущему активному элементу страницы и навигационного меню
 */
function fullPageScroll(options) {
    ...
    let initialActivePage = options.initialActivePage ? options.initialActivePage : 0;
    let arrayPages = [];
    let arrayMarkers = [];
